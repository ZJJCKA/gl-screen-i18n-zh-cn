...
- 字体：IBM Plex Sans SC，使用设备原字体的纵向指标重新生成；四个字体角色共享一份字形子集，覆盖固定界面、短信正文、四个国内运营商名称、3500 个现代汉语高频字以及 `config/extra-glyphs.txt` 中可扩展的中文 SSID 白名单；保留小屏 TrueType hinting，删除横向界面不使用的竖排度量和 OpenType 布局表
...
2. 安装中文语言文件与 1 个静态界面字形子集，四个字体角色共用该字库；
3. 覆盖当前 `default` 语言文件；
